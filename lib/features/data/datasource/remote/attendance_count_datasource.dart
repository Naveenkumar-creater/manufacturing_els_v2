...
abstract class AttendanceCountDataSOurce {
  Future<AttendanceCountModel> getAttCount(int id,int deptid,int psid, String token, int orgid);
}

class AttendanceCountDataSOurceImpl extends AttendanceCountDataSOurce {
  @override
  Future<AttendanceCountModel> getAttCount(int id,int deptid,int psid, String token, int orgid) async {
    ApiRequestDataModel requestBody = ApiRequestDataModel(
        apiFor: "attendance_count_v1", clientAuthToken: token, processId: id,deptId: deptid,psId: psid, orgid: orgid);
